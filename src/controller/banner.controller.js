...
export default class BannerController {
    static async getAll(req, res) {
        try {
          ...
          connected.query(users, (err, result) => {
            ...
            if (!result[0]) return SendError(res, 404, EMessage.NotFound + " banner");
    ...
    static async insert(req, res) {
        try {
            ...
            const insert = "insert into banner (bUuid,title,detail,image,createdAt,updatedAt) values (?,?,?,?,?,?)"
            const bUuid = uuidv4();
        const datetime = new Date()
          ...
          .replace(/\..+/, "");
            ...
            connected.query(insert, [bUuid, title, detail, image_url, datetime, datetime], (err) => {
